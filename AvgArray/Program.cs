namespace AvgArray
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                // Welcome message
                WelcomeApp("average of all integer array elements accepted by the user");
                ...
                if (!ReadArray(nValues))
                    return;
                // To calculate the average of all elements in the array
                Print($"The average is: {AvgArray(nValues)}");
        ...
        // 8) this method to calculate the avg of all elements in the array
        static double AvgArray(int[] elements)
        {
            long nSum = 0;
            for (int nCounter = 0; nCounter <= elements.Length - 1; nCounter++)
                nSum += elements[nCounter];
            return nSum / (double)elements.Length;
        }
